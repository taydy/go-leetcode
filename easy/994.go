...
// 如果正方向上的值为 1 或者正方向上的值大于当前值+1，则执行广度搜索
func orangesRottingDFS(grid [][]int, i, j, depth int) {
	grid[i][j] = depth

	if i > 0 && (grid[i-1][j] == 1 || grid[i-1][j]-grid[i][j] > 1) {
		orangesRottingDFS(grid, i-1, j, depth+1)
	}
	if i < len(grid)-1 && (grid[i+1][j] == 1 || grid[i+1][j]-grid[i][j] > 1) {
		orangesRottingDFS(grid, i+1, j, depth+1)
	}
	if j > 0 && (grid[i][j-1] == 1 || grid[i][j-1]-grid[i][j] > 1) {
		orangesRottingDFS(grid, i, j-1, depth+1)
	}
